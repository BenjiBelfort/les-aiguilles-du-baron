...
const imagesJsonPath = path.join(__dirname, "src/data/photos.json");
...
const imageXml = images.map(img => {
  return `    <image:image>
      <image:loc>${baseUrl.replace(/\/$/, "")}${img.url}</image:loc>
...
console.log("✅ sitemap-images.xml généré avec success !");
